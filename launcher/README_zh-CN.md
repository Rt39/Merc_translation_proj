...
`<install>/AssetBundle/` 之间建立一个 NTFS mount-point junction，然后启动
`メルストM_app.exe`。幂等 —— 第二次运行发现 junction 已存在就直接启动。
...
Unity 用 exe 的基名推导 `_Data` 目录名 —— exe 改名为 `メルストM_app.exe`
之后它会去找 `メルストM_app_Data/`，所以数据目录的改名是必须的。
