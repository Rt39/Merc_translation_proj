...
在 `mercstoria/memorypack.py` 中以 `Reader` 类实现（也可通过 `from mercstoria import memorypack` 引用）。子对象用 `obj_header()` 剥 `memberCount` 字节（`0xFF` = null）。
...
**A. 提取 → JSON。** `mercstoria extract-story` 遍历每个 `StoryMasterData/`，解密、跑完整 MemoryPack Reader（只有 round-trip 字节级一致的 bundle 才会写入 JSON），并把章节/小节元数据（从 `StoryMasterData` + `ChapterMasterData` 拉来）一起写在每个 JSON 顶部。`mercstoria extract-misc` 用同一套 Reader/Writer 覆盖全部 15 个 master bundle（见 [`MASTERDATA_SCHEMA_GUIDE_zh-CN.md`](MASTERDATA_SCHEMA_GUIDE_zh-CN.md)）。约 4,000 剧情，约 120k 行对话，约 25 MB JSON，30 秒跑完。
...
Speaker 名同时出现在**两处**：场景的 `Speakers: string[]` 和在场角色的 `StorySceneCharacterYamlData.Key` / `DisplayName`。Toolkit 抽出的每剧情 JSON 同时暴露这两处 —— 编辑 JSON 后重打包时两处一并更新。
...
- **完整 schema 驱动的 (反)序列化器。** 最初写完弃用过：每次游戏更新都可能挪字段可空性或类型。后来 `dump.cs` 把 schema 落地为权威来源，于是又拣回来 —— `mercstoria/memorypack.py` 现在能让 4008/4013 个 story bundle 字节级 round-trip，外加全部 15 个 master bundle。当年的启发式扫描器留在 `scripts/extract_repack.py` 里作为兜底，给以后还没接 schema 的 bundle 用。
...
| 路径 | 用途 |
|---|---|
| `scripts/extract_repack.py` | 统一 CLI（通过 `mercstoria <subcmd>`）：`extract` / `extract-story` / `extract-misc` / `repack` / `repack-story` / `repack-misc` / `test-repack` |
| `mercstoria/memorypack.py` | 解密 + 完整 `StoryYamlData` 与 15 种 master record 的 MemoryPack `Reader`/`Writer` |
| `scripts/check_roundtrip.py` | `mercstoria check-roundtrip [N]` —— 对前 N 个 story bundle 做 Reader/Writer 一致性检查 |
| `scripts/deploy.py` | `mercstoria deploy` —— 把 `repacked_bundles/` 推回实时缓存（自动优先游戏目录，回退 LocalLow） |
| `scripts/bundle_cache.py` | `mercstoria bundle-cache` —— 把 LocalLow CDN 缓存打包进游戏目录，便于分发 |
