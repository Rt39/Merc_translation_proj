...
def _t(key: str, **kw) -> str:
    return _MESSAGES[key].format(**kw)
...
def _copy_tree(src: Path, dst: Path, total_files: int, total_bytes: int) -> tuple[int, int, int]:
...
def main() -> int:
    ...
    print(_t("title"))
    print(_t("source", path=src))
    print(_t("dest",   path=dst))
    print()

    if not src.is_dir():
        print(_t("source_missing"))
        return 1
    ...
    if not dst.parent.is_dir():
        print(_t("dest_missing"))
        return 1

    print(_t("scanning"), end="", flush=True)
    n_files, n_bytes = _scan(src)
    print(_t("scan_result",
             count=n_files, bytes_gb=n_bytes / 1_073_741_824))
    print()

    if not args.yes:
        resp = input(_t("confirm")).strip().lower()
        if resp and resp not in ("y", "yes"):
            print(_t("aborted"))
            return 0

    start = time.monotonic()
    copied, skipped, copied_bytes = _copy_tree(src, dst, n_files, n_bytes)
    elapsed = time.monotonic() - start

    if skipped:
        print(_t("skipped", count=skipped))
    print(_t("done",
             copied=copied,
